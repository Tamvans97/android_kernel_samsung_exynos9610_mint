...
VERIFY_TOOLCHAIN() {
    ...
    if [ -d "$TOOLCHAIN" ]; then
        ...
        if [[ "$OS_ARCH" == "aarch64" || "$IS_PROOT" == true ]]; then
            ...
            URL_RELEASE="https://github.com/Defir97/proton-clang/releases/download/beta-v1.0.0/proton-clang-13-aarch64.tar.gz"
            
            if ! wget -q --show-progress --continue "$URL_RELEASE" -O clang_temp.tar.gz; then
                script_echo "E: Download Failed!"
                return 1
            fi
            ...
            if tar -xzf clang_temp.tar.gz -C "$TOOLCHAIN" --strip-components=1; then
                ...
                script_echo "E: Failed Extracting!"
                return 1
            fi
        else
...
SET_LOCALVERSION() {
    ...
    *)        export LOCALVERSION=" - Mint Beta $GITHUB_RUN_NUMBER-android15"
    esac
}
SET_ZIPNAME() {
    ...
    $BUILD_KERNEL_KSU && ROOT_SOLUTION="-KSUN"
    ...
    if [[ $BUILD_KERNEL_BRANCH == mainline ]]; then
        FILE_NAME="Mint-${MINT_VERSION}.A15.${MINT_VARIANT}${ONEUI_VERSION}${ROOT_SOLUTION}_${BUILD_DEVICE_NAME^}.zip"
    else
        FILE_NAME="MintBeta-${MINT_VERSION}.A15.${MINT_VARIANT}${ONEUI_VERSION}-${MINT_SELINUX}${ROOT_SOLUTION}_${BUILD_DEVICE_NAME^}.${MINT_TYPE}.zip"
    fi
}
